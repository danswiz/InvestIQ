...
# ============== CONFIGURATION ==============
EMAIL_SENDER = "danswizbot@gmail.com"
...
def generate_html_report(session="pm"):
    ...
    if len(sys.argv) > 2:
        # Append verification note if provided as 2nd arg
        verification_note = sys.argv[2]
        html.append(f"""
        <div style="margin-top: 20px; padding: 10px; background-color: #e8f0fe; border-radius: 4px; font-size: 12px; color: #1a73e8;">
            <strong>✅ Verification:</strong> {verification_note}
        </div>
        """)
